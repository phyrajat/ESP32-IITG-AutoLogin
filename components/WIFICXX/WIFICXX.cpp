...
namespace WIFI
{
	...
	{
		memcpy(_wifi_cfg.sta.ssid, ssid, std::min(strlen(ssid), (size_t) sizeof(_wifi_cfg.sta.ssid)));

		memcpy(_wifi_cfg.sta.password, password, std::min(strlen(password), (size_t) sizeof(_wifi_cfg.sta.password)));
	}
	...
	esp_err_t Wifi::_get_mac(void)
	{
		...
		if (ESP_OK == status)
		{
			snprintf(_mac_addr_cstr, sizeof(_mac_addr_cstr), "%02X%02X%02X%02X%02X%02X", mac_byte_buffer[0], mac_byte_buffer[1], mac_byte_buffer[2], mac_byte_buffer[3], mac_byte_buffer[4], mac_byte_buffer[5]);
		}
